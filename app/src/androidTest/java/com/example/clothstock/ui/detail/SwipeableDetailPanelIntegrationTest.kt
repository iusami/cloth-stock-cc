...
class SwipeableDetailPanelIntegrationTest {
    ...
    fun swipeableDetailPanel_上スワイプ時_パネルが非表示になる() {
        ...
        ActivityScenario.launch<DetailActivity>(intent).use { scenario ->
            ...
            scenario.onActivity { activity ->
                ...
                panel?.let {
                    idlingResource = AnimationIdlingResource(it, "SwipeUpAnimation")
            ...
            try {
                ...
                // Then: パネルが非表示状態になる（IdlingResourceでアニメーション完了を待機）
    ...
    fun swipeableDetailPanel_下スワイプ時_パネルが表示される() {
        ...
        ActivityScenario.launch<DetailActivity>(intent).use { scenario ->
            ...
            scenario.onActivity { activity ->
                val panel = activity.findViewById<android.view.View>(R.id.swipeableDetailPanel)
                panel?.let {
                    idlingResource = AnimationIdlingResource(it, "SwipeDownAnimation")
                    registerIdlingResources(idlingResource)
                }
            }
            ...
            try {
                ...
                // Then: パネルが完全表示状態になる（IdlingResourceでアニメーション完了を待機）
                onView(withId(R.id.swipeableDetailPanel))
                    .check(matches(isDisplayed()))
    ...
    fun swipeableDetailPanel_スワイプハンドルタップ時_表示状態が切り替わる() {
        ...
        ActivityScenario.launch<DetailActivity>(intent).use {
            ...
            // Then: パネルが非表示になる
            waitForTimeout(500) // アニメーション完了待ち
    ...
    fun swipeableDetailPanel_高速スワイプ時_即座に状態変化する() {
        ...
        ActivityScenario.launch<DetailActivity>(intent).use {
            ...
            waitForTimeout(600) // フリングアニメーション完了待ち
    ...
    /**
     * SwipeableDetailPanelのアニメーション完了を待機するヘルパーメソッド
     */
    private fun ActivityScenario<DetailActivity>.waitForPanelAnimation(actionName: String) {
        var idlingResource: AnimationIdlingResource? = null
        this.onActivity { activity ->
            val panel = activity.findViewById<android.view.View>(R.id.swipeableDetailPanel)
            panel?.let {
                idlingResource = AnimationIdlingResource(it, actionName)
                registerIdlingResources(idlingResource)
            }
        }
        // IdlingResourceは自動的にアニメーション完了を待機
        // クリーンアップはtry-finallyで行う
        idlingResource?.let { unregisterIdlingResources(it) }
    }

    /**
     * タイムアウト待機のヘルパーメソッド（最後の手段として使用）
     */
    private fun waitForTimeout(timeoutMs: Long) {
        val timeoutIdlingResource = AnimationIdlingResource.timeout(timeoutMs)
        registerIdlingResources(timeoutIdlingResource)
        // アクションの実行
        unregisterIdlingResources(timeoutIdlingResource)
    }
